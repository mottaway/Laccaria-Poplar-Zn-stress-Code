...
CATALASED14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 16) 
head(CATALASED14)

CATALASED14$Tissue = as.factor(CATALASED14$Tissue)
CATALASED14$Zn <- factor(CATALASED14$Zn, levels=c("Control", "1.5 mM Zn"))
CATALASED14$`CATALASED14log2` = as.numeric(CATALASED14$`CATALASED14log2`)
CATALASED14$`CATALASED14Mean` = as.numeric(CATALASED14$`CATALASED14Mean`)
CATALASED14$`CATALASED14SE` = as.numeric(CATALASED14$`CATALASED14SE`)

Tissue <- CATALASED14$Tissue
Zn <- CATALASED14$Zn
log2 <- CATALASED14$`CATALASED14log2`
Mean<- CATALASED14$`CATALASED14Mean`
SE<- CATALASED14$`CATALASED14SE`
...
w <- summary(x)
w
capture.output(w ,file="CATALASE D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "CATALASE D14 Tukey paper.csv")

library(ggplot2)
library(ggsignif)

list <- data.frame (y_position = c(NA, 1.5, NA, NA, NA, 2), xmin= c(NA, 0.75, NA, NA, NA, 1.25), xmax= c(NA, 1.25, NA, NA, NA, 2.25))
df <- Statistics %>%   mutate (list)
df

figure <-
ggplot(CATALASED14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "Catalase", x= " ", y = "Log2 (Relative expression)") +
