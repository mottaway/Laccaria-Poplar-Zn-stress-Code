...
ZIPDD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 5) 
head(ZIPDD14)

ZIPDD14$Tissue = as.factor(ZIPDD14$Tissue)
ZIPDD14$Zn <- factor(ZIPDD14$Zn, levels=c("Control", "1.5 mM Zn"))
ZIPDD14$`ZIPDD14log2` = as.numeric(ZIPDD14$`ZIPDD14log2`)
ZIPDD14$`ZIPDD14Mean` = as.numeric(ZIPDD14$`ZIPDD14Mean`)
ZIPDD14$`ZIPDD14SE` = as.numeric(ZIPDD14$`ZIPDD14SE`)

Tissue <- ZIPDD14$Tissue
Zn <- ZIPDD14$Zn
log2 <- ZIPDD14$`ZIPDD14log2`
Mean<- ZIPDD14$`ZIPDD14Mean`
SE<- ZIPDD14$`ZIPDD14SE`
...
w <- summary(x)
w
capture.output(w ,file="ZIPD D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "ZIPD D14 Tukey paper.csv")
...
figure <-
ggplot(ZIPDD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "ZIP-D", x= " ", y = "Log2 (Relative expression)") +
