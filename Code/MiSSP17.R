...
MISSP17D14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 19) 
head(MISSP17D14)

MISSP17D14$Tissue = as.factor(MISSP17D14$Tissue)
MISSP17D14$Zn <- factor(MISSP17D14$Zn, levels=c("Control", "1.5 mM Zn"))
MISSP17D14$`MISSP17D14log2` = as.numeric(MISSP17D14$`MISSP17D14log2`)
MISSP17D14$`MISSP17D14Mean` = as.numeric(MISSP17D14$`MISSP17D14Mean`)
MISSP17D14$`MISSP17D14SE` = as.numeric(MISSP17D14$`MISSP17D14SE`)

Tissue <- MISSP17D14$Tissue
Zn <- MISSP17D14$Zn
log2 <- MISSP17D14$`MISSP17D14log2`
Mean<- MISSP17D14$`MISSP17D14Mean`
SE<- MISSP17D14$`MISSP17D14SE`
...
w <- summary(x)
w
capture.output(w ,file="MISSP17 D14 Anova Paper.doc") #store data
...
Statistics <- z[4:5]
Statistics
write.csv2(Statistics, file = "MISSP17 D14 Tukey paper.csv")
...
list <- data.frame (y_position = c(2.5, NA, NA, NA, NA, 2), xmin= c(0.75, NA, NA, NA, NA, 1.25), xmax= c(1.75, NA, NA, NA, NA, 2.25))
df <- Statistics %>%   mutate (list)
df

figure <-
ggplot(MISSP17D14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "MiSSP17", x= " ", y = "Log2 (Relative expression)") +
