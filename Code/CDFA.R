...
CDFAD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 7) 
head(CDFAD14)

CDFAD14$Tissue = as.factor(CDFAD14$Tissue)
CDFAD14$Zn <- factor(CDFAD14$Zn, levels=c("Control", "1.5 mM Zn"))
CDFAD14$`CDFAD14log2` = as.numeric(CDFAD14$`CDFAD14log2`)
CDFAD14$`CDFAD14Mean` = as.numeric(CDFAD14$`CDFAD14Mean`)
CDFAD14$`CDFAD14SE` = as.numeric(CDFAD14$`CDFAD14SE`)

Tissue <- CDFAD14$Tissue
Zn <- CDFAD14$Zn
log2 <- CDFAD14$`CDFAD14log2`
Mean<- CDFAD14$`CDFAD14Mean`
SE<- CDFAD14$`CDFAD14SE`
...
w <- summary(x)
w
capture.output(w ,file="CDFA D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "CDFA D14 Tukey paper.csv")

library(ggplot2)
library(ggsignif)

list <- data.frame (y_position = c(2.5, 1.5, NA, NA, NA, 2), xmin= c(0.75, 0.75, NA, NA, NA, 1.25), xmax= c(1.75, 1.25, NA, NA, NA, 2.25))
df <- Statistics %>%   mutate (list)
df


figure <-
ggplot(CDFAD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "CDF-A", x= " ", y = "Log2 (Relative expression)") +
