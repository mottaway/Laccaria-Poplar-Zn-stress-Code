...
MNFESOD3D14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 13) 
head(MNFESOD3D14)

MNFESOD3D14$Tissue = as.factor(MNFESOD3D14$Tissue)
MNFESOD3D14$Zn <- factor(MNFESOD3D14$Zn, levels=c("Control", "1.5 mM Zn"))
MNFESOD3D14$`MNFESOD3D14log2` = as.numeric(MNFESOD3D14$`MNFESOD3D14log2`)
MNFESOD3D14$`MNFESOD3D14Mean` = as.numeric(MNFESOD3D14$`MNFESOD3D14Mean`)
MNFESOD3D14$`MNFESOD3D14SE` = as.numeric(MNFESOD3D14$`MNFESOD3D14SE`)

Tissue <- MNFESOD3D14$Tissue
Zn <- MNFESOD3D14$Zn
log2 <- MNFESOD3D14$`MNFESOD3D14log2`
Mean<- MNFESOD3D14$`MNFESOD3D14Mean`
SE<- MNFESOD3D14$`MNFESOD3D14SE`
...
w <- summary(x)
w
capture.output(w ,file="MNFESOD3 D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "MNFESOD3 D14 Tukey paper.csv")

library(ggplot2)
library(ggsignif)

list <- data.frame (y_position = c(NA, NA, NA, NA, NA,2), xmin= c(NA, NA, NA, NA, NA,1.25), xmax= c(NA, NA, NA, NA, NA,2.25))
df <- Statistics %>%   mutate (list)
df


figure <-
ggplot(MNFESOD3D14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "Mn/Fe SOD3", x= " ", y = "Log2 (Relative expression)") +
