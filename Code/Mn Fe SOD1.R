...
getwd() 
MNFESOD1D14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 11) 
head(MNFESOD1D14)

MNFESOD1D14$Tissue = as.factor(MNFESOD1D14$Tissue)
MNFESOD1D14$Zn <- factor(MNFESOD1D14$Zn, levels=c("Control", "1.5 mM Zn"))
MNFESOD1D14$`MNFESOD1D14log2` = as.numeric(MNFESOD1D14$`MNFESOD1D14log2`)
MNFESOD1D14$`MNFESOD1D14Mean` = as.numeric(MNFESOD1D14$`MNFESOD1D14Mean`)
MNFESOD1D14$`MNFESOD1D14SE` = as.numeric(MNFESOD1D14$`MNFESOD1D14SE`)

Tissue <- MNFESOD1D14$Tissue
Zn <- MNFESOD1D14$Zn
log2 <- MNFESOD1D14$`MNFESOD1D14log2`
Mean<- MNFESOD1D14$`MNFESOD1D14Mean`
SE<- MNFESOD1D14$`MNFESOD1D14SE`
...
w <- summary(x)
w
capture.output(w ,file="MNFESOD1 D14 Anova Paper.doc") #store data
...
Statistics <- z[4:5]
Statistics
write.csv2(Statistics, file = "MNFESOD1 D14 Tukey paper.csv")
...
figure <-
ggplot(MNFESOD1D14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "Mn/Fe SOD1", x= " ", y = "Log2 (Relative expression)") +
