...
#read in your file and check it
MNFESOD2D14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 12) 
head(MNFESOD2D14)

MNFESOD2D14$Tissue = as.factor(MNFESOD2D14$Tissue)
MNFESOD2D14$Zn <- factor(MNFESOD2D14$Zn, levels=c("Control", "1.5 mM Zn"))
MNFESOD2D14$`MNFESOD2D14log2` = as.numeric(MNFESOD2D14$`MNFESOD2D14log2`)
MNFESOD2D14$`MNFESOD2D14Mean` = as.numeric(MNFESOD2D14$`MNFESOD2D14Mean`)
MNFESOD2D14$`MNFESOD2D14SE` = as.numeric(MNFESOD2D14$`MNFESOD2D14SE`)

Tissue <- MNFESOD2D14$Tissue
Zn <- MNFESOD2D14$Zn
log2 <- MNFESOD2D14$`MNFESOD2D14log2`
Mean<- MNFESOD2D14$`MNFESOD2D14Mean`
SE<- MNFESOD2D14$`MNFESOD2D14SE`
...
w <- summary(x)
w
capture.output(w ,file="MNFESOD2 D14 Anova Paper.doc") #store data
...
Statistics <- z[4:5]
Statistics
write.csv2(Statistics, file = "MNFESOD2 D14 Tukey paper.csv")

library(ggplot2)

figure <-
ggplot(MNFESOD2D14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
geom_bar(stat='identity', position="dodge") +
geom_errorbar( aes(ymin=Mean-SE, ymax= Mean+SE ), width=0.1, size=2, position=position_dodge(1)) +
labs(title = " Mn/Fe SOD2", x= " ", y = "Log2 (Relative expression)") +
