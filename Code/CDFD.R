...
CDFDD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 10) 
head(CDFDD14)

CDFDD14$Tissue = as.factor(CDFDD14$Tissue)
CDFDD14$Zn <- factor(CDFDD14$Zn, levels=c("Control", "1.5 mM Zn"))
CDFDD14$`CDFDD14log2` = as.numeric(CDFDD14$`CDFDD14log2`)
CDFDD14$`CDFDD14Mean` = as.numeric(CDFDD14$`CDFDD14Mean`)
CDFDD14$`CDFDD14SE` = as.numeric(CDFDD14$`CDFDD14SE`)

Tissue <- CDFDD14$Tissue
Zn <- CDFDD14$Zn
log2 <- CDFDD14$`CDFDD14log2`
Mean<- CDFDD14$`CDFDD14Mean`
SE<- CDFDD14$`CDFDD14SE`
...
w <- summary(x)
w
capture.output(w ,file="CDFD D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "CDFD D14 Tukey paper.csv")

library(ggplot2)
library(ggsignif)

figure <-
ggplot(CDFDD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
geom_bar(stat='identity', position="dodge") +
geom_errorbar( aes(ymin=Mean-SE, ymax= Mean+SE ), width=0.1, size=2, position=position_dodge(1)) +
labs(title = "CDF-D", x= " ", y = "Log2 (Relative expression)") +
