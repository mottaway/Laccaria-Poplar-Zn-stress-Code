...
CDFCD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 9) 
head(CDFCD14)

CDFCD14$Tissue = as.factor(CDFCD14$Tissue)
CDFCD14$Zn <- factor(CDFCD14$Zn, levels=c("Control", "1.5 mM Zn"))
CDFCD14$`CDFCD14log2` = as.numeric(CDFCD14$`CDFCD14log2`)
CDFCD14$`CDFCD14Mean` = as.numeric(CDFCD14$`CDFCD14Mean`)
CDFCD14$`CDFCD14SE` = as.numeric(CDFCD14$`CDFCD14SE`)

Tissue <- CDFCD14$Tissue
Zn <- CDFCD14$Zn
log2 <- CDFCD14$`CDFCD14log2`
Mean<- CDFCD14$`CDFCD14Mean`
SE<- CDFCD14$`CDFCD14SE`
...
w <- summary(x)
w
capture.output(w ,file="CDFC D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "CDFC D14 Tukey paper.csv")
...
figure <-
ggplot(CDFCD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "CDF-C", x= " ", y = "Log2 (Relative expression)") +
