...
ZIPCD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 4) 
head(ZIPCD14)

ZIPCD14$Tissue = as.factor(ZIPCD14$Tissue)
ZIPCD14$Zn <- factor(ZIPCD14$Zn, levels=c("Control", "1.5 mM Zn"))
ZIPCD14$`ZIPCD14log2` = as.numeric(ZIPCD14$`ZIPCD14log2`)
ZIPCD14$`ZIPCD14Mean` = as.numeric(ZIPCD14$`ZIPCD14Mean`)
ZIPCD14$`ZIPCD14SE` = as.numeric(ZIPCD14$`ZIPCD14SE`)

Tissue <- ZIPCD14$Tissue
Zn <- ZIPCD14$Zn
log2 <- ZIPCD14$`ZIPCD14log2`
Mean<- ZIPCD14$`ZIPCD14Mean`
SE<- ZIPCD14$`ZIPCD14SE`
...
w <- summary(x)
w
capture.output(w ,file="ZIPC D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "ZIPC D14 Tukey paper.csv")

library(ggplot2)
library(ggsignif)

list <- data.frame (y_position = c(NA, 1.5, NA, NA, 1.5, NA), xmin= c(NA, 0.75, NA, NA, 1.75, NA), xmax= c(NA, 1.25, NA, NA, 2.25, NA))
df <- Statistics %>%   mutate (list)
df

figure <-
ggplot(ZIPCD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "ZIP-C", x= " ", y = "Log2 (Relative expression)") +
