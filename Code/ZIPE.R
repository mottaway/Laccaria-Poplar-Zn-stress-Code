...
ZIPED14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 6) 
head(ZIPED14)

ZIPED14$Tissue = as.factor(ZIPED14$Tissue)
ZIPED14$Zn <- factor(ZIPED14$Zn, levels=c("Control", "1.5 mM Zn"))
ZIPED14$`ZIPED14log2` = as.numeric(ZIPED14$`ZIPED14log2`)
ZIPED14$`ZIPED14Mean` = as.numeric(ZIPED14$`ZIPED14Mean`)
ZIPED14$`ZIPED14SE` = as.numeric(ZIPED14$`ZIPED14SE`)

Tissue <- ZIPED14$Tissue
Zn <- ZIPED14$Zn
log2 <- ZIPED14$`ZIPED14log2`
Mean<- ZIPED14$`ZIPED14Mean`
SE<- ZIPED14$`ZIPED14SE`
...
w <- summary(x)
w
capture.output(w ,file="ZIPE D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "ZIPE D14 Tukey paper.csv")
...
figure <-
ggplot(ZIPED14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "ZIP-E", x= " ", y = "Log2 (Relative expression)") +
scale_color_manual (values = c("blue1", "darkorange")) +
scale_fill_manual (values = c("blue1", "darkorange")) +
scale_y_continuous(limits = c(-2, 2), breaks=seq(-2, 4, by = 1)) +
theme_light() +
