...
ZIPBD14 = readxl::read_excel("PaperJSMOKV.xlsx", sheet = 3) 
head(ZIPBD14)

ZIPBD14$Tissue = as.factor(ZIPBD14$Tissue)
ZIPBD14$Zn <- factor(ZIPBD14$Zn, levels=c("Control", "1.5 mM Zn"))
ZIPBD14$`ZIPBD14log2` = as.numeric(ZIPBD14$`ZIPBD14log2`)
ZIPBD14$`ZIPBD14Mean` = as.numeric(ZIPBD14$`ZIPBD14Mean`)
ZIPBD14$`ZIPBD14SE` = as.numeric(ZIPBD14$`ZIPBD14SE`)

Tissue <- ZIPBD14$Tissue
Zn <- ZIPBD14$Zn
log2 <- ZIPBD14$`ZIPBD14log2`
Mean<- ZIPBD14$`ZIPBD14Mean`
SE<- ZIPBD14$`ZIPBD14SE`
...
w <- summary(x)
w
capture.output(w ,file="ZIPB D14 Anova Paper.doc") #store data
...
write.csv2(Statistics, file = "ZIPB D14 Tukey paper.csv")
...
figure <-
ggplot(ZIPBD14, aes(x = Tissue, y = Mean, fill=Zn, color = Zn)) +
...
labs(title = "ZIP-B", x= " ", y = "Log2 (Relative expression)") +
